...
#########################
def calculate_recipients(subscribers, unsubscribers)
  # Using the SUBSCRIBERS and UNSUBSCRIBED arrays,
  # write a method that will return an array of only the subscribers who haven't unsubscribed
  array = []
  subscribers.each do |element|
    if unsubscribers.include?(element)
    else
      array << element
    end
  end
...
def print_recipients
  ...
  p calculate_recipients(SUBSCRIBERS,UNSUBSCRIBED)
...
def print_one_article(article)
  ...
  first_article = []
  # article[0].each do |element|
  #   first_article << article[0][element]
  # end
  first_article << article[0][:title]
  first_article << article[0][:author]
  first_article << article[0][:text]
  p first_article
...
def print_many_articles(articles)
  ...
  all_articles = []
  articles.each do |array_index|
    all_articles << articles[array_index][:title]
    all_articles << articles[array_index][:author]
    all_articles << articles[array_index][:text]
  end
p all_articles
end
